...
local function a(b)
    ...
    if c == 'table' then
        d = {}
        for e, f in next, b, nil do d[a(e)] = a(f) end
        setmetatable(d, a(getmetatable(b)))
    else d = b end
    return d
end

local function cp(k, name, tooltip)
    local l = k[cps]
    l.i18n_en_humanname = name
    l.i18n_en_tooltip = tooltip
    l.techlevel = 2
    return l
end

local function es(k, bt, ec, mc, hp, acc, spd, tr, exp, sight)
    k.buildtime = bt; k.energycost = ec; k.metalcost = mc; k.health = hp
    k.maxacc = acc; k.maxdec = acc; k.speed = spd; k.turnrate = tr
    k.explodeas = exp; k.selfdestructas = exp .. "Selfd"
    k.sightdistance = sight
    k.movementclass = "EPICSHIP"
    k.radardistance = nil
end

local function fd(k, dm, dd, hm, hd)
    local r = k.featuredefs
    if r then
        if r.dead then r.dead.metal = dm; r.dead.damage = dd end
        if r.heap then r.heap.metal = hm; r.heap.damage = hd end
    end
end

local function rb(k, bd)
    k.builder = true; k.canrepair = true; k.canreclaim = false; k.canassist = true
    k.workertime = 200; k.builddistance = bd
    k.terraformspeed = nil
end

local ids = {
    leganavyflagship = "leganavythalassat3",
    leganavymissileship = "leganavymissileshipt3",
    leganavyantinukecarrier = "leganavyrepairship",
    legassistdrone = "legrepairdrone",
}

local i = {

    leganavyflagship = function(j)
        local k = a(j)
        es(k, 76000, 90000, 4500, 19850, 0.045, 44, 260, "hugeexplosiongeneric", 700)
        k.energymake = nil
        k.mass = nil
        k.nochasecategory = "VTOL UNDERWATER"
        cp(k, "Epic Thalassa", "Broadside Galley Battleship").paralyzemultiplier = nil
        k.icontype = "armbats"
        k[wds].targeting_system.range = 850
        local w1 = a(uDefs.legaskirmtank[wds].legmgplasma)
        k[wds].leg_experimental_railgun = w1
        w1.name = "Heavy Rotary Plasma Broadside Cannon"
        w1.areaofeffect = 80
        w1.burst = 3
        w1.burstrate = 0.25
        w1.range = 750
        w1.reloadtime = 2
        w1.weaponvelocity = 500
        w1.damage = { default = 300, vtol = 50 }
        local w2 = a(uDefs.legsrailt4[wds].railgunt2)
        k[wds].leg_experimental_heatray = w2
        w2.name = "Heavy Rail Accelerator"
        w2.areaofeffect = 64
        w2.firetolerance = 50
        w2.energypershot = 1500
        w2.impactonly = nil
        w2.range = 850
        w2.reloadtime = 6
        w2.stockpile = nil
        w2.stockpiletime = nil
        w2[cps].stockpilelimit = nil
        w2.damage = { default = 1000, commanders = 200 }
        local n = k.weapons
        n[4].maindir = "1 0 0"; n[4].maxangledif = 160
        n[5].maindir = "-1 0 0"; n[5].maxangledif = 160
        n[6].maindir = "1 0 0"; n[6].maxangledif = 160
        n[7].maindir = "-1 0 0"; n[7].maxangledif = 160
        fd(k, 3500, 22000, 1750, 11000)
        return k
    end,

    leganavymissileship = function(j)
        local k = a(j)
        es(k, 110000, 105000, 8500, 9500, 0.008, 39, 90, "hugeBuildingExplosionGeneric", 550)
        cp(k, "Epic Ultor", "Long-Range VLS Arsenal Ship").maxrange = "3300"
        k.icontype = "leganavyartyship"
        local w3 = a(uDefs.cortron[wds].cortron_weapon)
        k[wds].leg_salvo_vertical_rocket = w3
        w3.name = "Vertical-Launch Ground Attack Missile Cells"
        w3.areaofeffect = 300
        w3.burst = 2
        w3.burstrate = 0.033
        w3.collideenemy = true
        w3.commandfire = nil
        w3.craterareaofeffect = 300
        w3.energypershot = 4500
        w3.flighttime = 15
        w3.interceptedbyshieldtype = nil
        w3.metalpershot = 0
        w3.range = 3300
        w3.reloadtime = 10
        w3.stockpile = nil
        w3.stockpiletime = nil
        w3.weaponacceleration = 300
        w3.weapontimer = 1
        w3.weaponvelocity = 1650
        w3[cps].nuclear = nil
        w3[cps].stockpilelimit = nil
        w3.damage = { default = 1000, commanders = 250 }
        local w4 = k[wds].leg_light_aa_missile
        w4.name = "Advanced Anti-Air Salvo Missile Launcher"
        w4.burst = 4
        w4.range = 900
        w4.reloadtime = 1.4
        w4.damage = { vtol = 30 }
        fd(k, 4250, 15000, 2125, 7500)
        return k
    end,

    leganavyantinukecarrier = function(j)
        local k = a(j)
        rb(k, 400)
        k.metalcost = 1600
        k.energycost = 26400
        k.buildtime = 38400
        k[wds].amd_rocket = nil
        k.energypershot = nil
        k.energymake = nil
        k.canattack = false
        k.radardistance = 0
